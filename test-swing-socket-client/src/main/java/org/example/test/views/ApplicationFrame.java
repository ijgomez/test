...
import org.example.test.events.OptionsEvent;
import org.example.test.views.client.ClientPanel;
...
import org.example.test.views.dialogs.DialogFactory;
...
public class ApplicationFrame extends JFrame implements ApplicationObserver {
    ...
    private void initComponents() {
        ...
        this.mainPanel = new ClientPanel();
	...
	public void listener(ApplicationEvent event) {
		if (event instanceof AboutEvent) {
			...
			dialog = DialogFactory.createAboutDialog(this);
			if (!dialog.isVisible()) {
				dialog.setVisible(true);
			}
			dialog.toFront();
			
		} else if (event instanceof OptionsEvent) {
			JDialog dialog;
			
			dialog = DialogFactory.createOptionsDialog(this);
			if (!dialog.isVisible()) {
				dialog.setVisible(true);
			}
			dialog.toFront();
