...
import org.example.test.helper.ByteArrayHelper;
...
public class TcpClient implements Runnable {
	...
	private static final Logger LOG = Logger.getLogger("TCP_LAYER");

	private long reconnectInterval = TcpClientParameters.INITIAL_RECONNECT_INTERVAL;

	/** Buffer de lectura de la aplicacion. */
	private ByteBuffer readBuffer = ByteBuffer.allocateDirect(TcpClientParameters.READ_BUFFER_SIZE);
	
	/** Buffer de escritura de la aplicacion. */
	private ByteBuffer writeBuffer = ByteBuffer.allocateDirect(TcpClientParameters.WRITE_BUFFER_SIZE); 
...
//			LOG.trace("writeBuffer: position=" + writeBuffer.position() + ", remaining=" + writeBuffer.remaining() + "(" + writeBuffer.hasRemaining() + "), capacity=" + writeBuffer.capacity());
			...
			if (writeBuffer.remaining() < buffer.remaining()) {
				...
				LOG.debug(String.format("TX (% 3d) -> %s", bytesTotal, ByteArrayHelper.toHexadecimal(result)));
...
//					Thread.sleep(reconnectInterval);
					LOG.debug("Status: " + connected.get());
					if (!connected.get().equals(Status.DISCONNECTING) && !connected.get().equals(Status.DISCONNECTED)) {
						if (reconnectInterval < TcpClientParameters.MAXIMUM_RECONNECT_INTERVAL) {
							LOG.warn("Reconnecting to " + address + " in " + TcpClientParameters.INITIAL_RECONNECT_INTERVAL + " ms...");
							Thread.sleep(TcpClientParameters.INITIAL_RECONNECT_INTERVAL);
							//reconnectInterval *= 2;
							reconnectInterval = reconnectInterval + TcpClientParameters.INITIAL_RECONNECT_INTERVAL;
							
						} else {
							LOG.error("Connection time exceeded (" + TcpClientParameters.MAXIMUM_RECONNECT_INTERVAL + " ms).");
...
//						Thread.sleep(INITIAL_RECONNECT_INTERVAL);
	...
	private void processConnect(SelectionKey key) throws Exception {
		...
		if (socketChannel.finishConnect()) {
			...
			reconnectInterval = TcpClientParameters.INITIAL_RECONNECT_INTERVAL;
	...
	private void processRead(SelectionKey key) throws Exception {
		...
		LOG.debug(String.format("RX (% 3d) <- %s", bytesTotal, ByteArrayHelper.toHexadecimal(result)));
